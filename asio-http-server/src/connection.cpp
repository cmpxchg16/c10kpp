...
{	
	file_.open(full_file_path_.c_str(), std::ios_base::binary | std::ios_base::ate);
...
void connection::handle_write_file(const boost::system::error_code& err)
{
	if (!err)
	{
		if (file_.eof()==false)
		{
			file_.read(file_buffer_.c_array(), (std::streamsize)file_buffer_.size());
			if (file_.gcount()<=0)
			{
				close();
				return;
			...
			boost::asio::async_write(socket_, 
				...
				boost::bind(&connection::handle_write_file, shared_from_this(),
				boost::asio::placeholders::error));
			if (err)
			{
    ...
    std::size_t bytes_transferred)
{
  if (!e)
  {
    ...
    if (result)
    {
      ...
      boost::asio::async_write(socket_, 
							   reply_.to_buffers(),
							   boost::bind(&connection::start_file_read, shared_from_this()));
    }
    else if (!result)
    {
