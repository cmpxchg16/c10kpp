...
const static char* ok_response =
		"HTTP/1.0 200 OK\n" 
		"Content-type: text/html\n" 
		"\n";

const static char* bad_request_response = 
...
void* sig_thread(void* arg)
{
	...
	void* res;
	for (std::vector<pthread_t>::iterator it = acceptors.begin() ; it != acceptors.end(); ++it)
	{
		pthread_cancel(*it);
		pthread_join(*it, &res);
	}
...
void* client_handler(void* arg)
{
	...
	{	
		...
		if (bytes_read > 0)
		{
			...
			{
				size_t sent = write(connection_fd, bad_request_response, strlen(bad_request_response));
			...
			else
			{
				...
				else
				{
				     ...
				     else
				     {
						 ...
						 size_t sent = sendfile(connection_fd, fd, NULL, sb.st_size);
...
{	
	...
	while (1) 
	{
		int connection = accept(a_args->sock, (struct sockaddr*)&client_addr, &addr_len);
...
int main(int argc, char* argv[])
{
	...
	{
		if ((sockfd = socket(tmpinfo->ai_family, tmpinfo->ai_socktype, tmpinfo->ai_protocol)) == -1) 
		{
			perror("socket");
			continue;
		}
